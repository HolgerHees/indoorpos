	...
	public T save( JdbcTemplateDAO jdbcTemplateDao, boolean refreshLastModified, T dto ){
		...
		try
		{
			...
			for( int i = 0; i < primaryValues.length; i++ )
			{
				...
				if( primaryColumns[i].getGetterConverter() != null ) primaryValues[i] = primaryColumns[i].getGetterConverter().invoke(primaryValues[i]);
			}
			...
			if( primaryColumns.length == 1 && !primaryColumns[0].isInsertable() )
			{
				...
				for( Column column : table.getColumns() )
				{
					...
					if( column.getGetterConverter() != null ) value = column.getGetterConverter().invoke(value);
					
					if( (!isInsert && !column.isUpdateable()) || (isInsert && !column.isInsertable()) ) continue;
				...
				if( isInsert )
				{
					jdbcTemplateDao.getJdbcTemplate().update( 
							"INSERT INTO " + table.getName() + " SET " + fieldSQL, 
				...
				else
				{
					...
					jdbcTemplateDao.getJdbcTemplate().update(
							"UPDATE " + table.getName() + " SET " + fieldSQL + " WHERE " + primaryColumns[0].getName() + " = ? ", 
			...
			else
			{
				...
				for( Column column : table.getColumns() )
				{
					...
					if( column.isInsertable() ) mappedInsertValueFields.put(column.getName(), value);
					
					if( column.isUpdateable() ) mappedUpdateValueFields.put(column.getName(), value);
				}
				...
				jdbcTemplateDao.getJdbcTemplate().update( 
						"INSERT INTO " + table.getName() + " SET " + insertFieldSQL + " ON DUPLICATE KEY UPDATE " + updateFieldSQL, 
				...
				return jdbcTemplateDao.getJdbcTemplate().queryForObject(
						"SELECT * FROM " + table.getName() + " WHERE " + whereFieldSQL.substring(5),
						primaryValues, 
						this );
			}
