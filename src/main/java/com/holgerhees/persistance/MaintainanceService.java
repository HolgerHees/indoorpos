	...
	public void createDatabaseSchema( boolean dropTable )
	{
		...
		if( dropTable ){
			...
			for( Table table: dropTableList ){
				...
				if( !existingTables.contains( table.getName() ) ) continue;
		...
		for( Table table: createTableList ){
			...
			Map<String,String> existingColumns = existingTables.contains( table.getName()) ? schemaDao.getExistingColumns( table.getName()) : new HashMap<String, String>();
			if( existingColumns.size() > 0 )
			{
				...
				for( Column column : table.getColumns() )
				{
					...
					{
						LOGGER.error("Changing of column definition of column '" + column.getName() + "' in table '" + table.getName() + "' not implemented");
						skipped = false;
					}
				...
				if( existingColumns.size() > 0 )
				{
					LOGGER.error("Removing of unsed column '"+ StringUtils
						.join(existingColumns.keySet(),", ")+"' in table '" + table.getName() + "' not implemented");
					skipped = false;
				}
